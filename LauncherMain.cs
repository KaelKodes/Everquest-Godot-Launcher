...
public partial class LauncherMain : Control
{
    ...
    private EqStyleDownloadBar _serverProgress;
    ...
    public override void _Ready()
    {
        ...
        _serverProgress = GetNode<EqStyleDownloadBar>("ServerPanel/ProgressBar");
    ...
    private async void OnServerActionExecute()
    {
        ...
        _serverProgress.EndWorkSuccess();
    ...
    private async Task<ClientReleaseZipAsset> FetchClientReleaseZipAssetRequiredAsync()
    {
        ...
        string json = await http.GetStringAsync(ClientGithubReleaseApiUrl);
        ...
        // Pick the first .zip we find on the latest release. If we ever need
        // to ship multiple zips per release (e.g. client + tools), reintroduce
        // a filename-preference filter here.
    ...
    private async Task<bool> InstallOrUpdateClientFromGitHubRelease(string installPath, int action, bool progressUsesFullBar = false)
    {
        ...
        try
        {
            asset = await FetchClientReleaseZipAssetRequiredAsync();
        }
        catch (Exception ex)
        {
    ...
    private async Task ExtractZipSafeWithProgressAsync(string zipPath, string destDir, int progressStart, int progressEnd)
    {
        await Task.Run(() =>
        {
